...
收到基本信息、读取质性资料全文后，**在进入六阶段之前**，询问研究者是否希望提供示范编码：
...
**前提**：已完成至少 3–4 份访谈的开放编码与主轴编码，有足够的类属积累。

选择性编码是扎根理论建构理论的关键步骤，目标是从所有类属中识别**核心类属**，并围绕核心类属整合其他类属，形成理论叙事。

### 步骤 1：识别核心类属候选

从累积的所有类属中，判断哪个类属（或哪几个类属的组合）最有可能成为核心类属。

核心类属的判断标准（S&C）：
- **出现频率高**：在多份访谈中反复出现
- **关联性强**：与其他大多数类属存在实质性联系
- **解释力强**：能统摄其他类属，说明"这里的核心现象是什么"
- **概念饱满**：属性和维度已有充分的数据支撑

输出候选核心类属（1–3个），并说明每个的理由。

### 步骤 2：构建故事线（Storyline）

围绕候选核心类属，用 1–2 段话写出分析故事线：

> "在这个研究中，核心现象是 [核心类属]。它的发生受到 [条件类属] 的影响，发生在 [情境类属] 的背景下。行动者通过 [行动/策略类属] 加以应对，最终产生了 [结果类属] 的结果。"

这是典范模型（Paradigm Model）的叙事版本，帮助研究者检验整合逻辑。
...
基于典范模型，提出 3–5 条可被后续数据检验的理论命题，格式：
